...
const getAmountofSpoling = async (stockName) => {
  try {
    // Create a new connection pool
    const pool = await sql.connect(config);
    const query = 'SELECT price FROM '+Constants.ProductTable+' WHERE stockname = @stockName';
    const result = await pool.request()
    ...
    if (result.recordset.length > 0) {
      ...
      if (!isNaN(numericPrice)) {
        console.log('Amount retrieved from database:', numericPrice);
        return numericPrice;
      } 
...
const ReduceQnt = async (stockName, quantity) => {
  try {
    // Create a new connection pool
    const pool = await sql.connect(config);
    const query = 'SELECT * FROM ' + Constants.ProductTable + ' WHERE stockname = @stockName';
    const result = await pool.request()
      .input('stockName', stockName)
      .query(query);

    if (result.recordset.length > 0) {
      const qnt = result.recordset[0].quantity;
      const price = result.recordset[0].price;

      if (qnt >= quantity) {
        const updatedQuantity = qnt - quantity;
        
        // Update quantity in the product table
        const updateQuery = 'UPDATE ' + Constants.ProductTable + ' SET quantity = @updatedQuantity WHERE stockname = @stockName';
        await pool.request()
          .input('updatedQuantity', updatedQuantity)
          .input('stockName', stockName)
          .query(updateQuery);
      
        // Log and perform additional operations
        console.log('Quantity reduced');
        const totalPrice = quantity * parseFloat(price);
        const stockAmount = 'stockamount';
        const expenses = 'expenses';
        await mdb.increaseAmount(expenses, totalPrice);
        console.log('Expenses increased');
        await mdb.decreaseAmount(stockAmount, totalPrice);
        console.log('Stock amount decreased');

        return 0; // Return 0 for success
      } else {
        console.log('Quantity entered is more than available');
        console.log('Available quantity is', qnt);
        return 2; // Return 2 for insufficient quantity
      }
    } else {
      console.log('No such product available');
      return 1; // Return 1 for product not found
    }
...
// write function for 
//module.exports = { getAmountofSpoling };
